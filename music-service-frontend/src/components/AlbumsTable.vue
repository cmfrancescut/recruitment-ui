...
async function loadPage() {
  await store.fetchAlbums().then(() => {
    albums.value = store.albums
    albumsPagination.value = store.albumPagination
    loading.value = false
  })
}
...
async function pageSelection(page) {
  if (currentPage.value !== page) {
    loading.value = true
    await store.fetchAlbums(10, page).then(() => {
      albumsPagination.value = store.albumPagination
...
function sort() {
  store.sortAlbums(sortAsc.value)
  ...
  <div class="prose">
    ...
    <div v-else>
      <table>
        ...
        <tr v-for="album in albums" class="odd:bg-gray-100">
          <td class="pl-2">{{ album.name }}</td>
        </tr>
      </table>
      <div class="flex space-x-5 gap-5 no-prose">
        ...
        <button
          class="border border-1 rounded-lg px-5 py-2 hover:text-white hover:bg-purple-700 disabled:cursor-not-allowed disabled:hover:no-underline disabled:hover:text-gray-300 disabled:text-gray-300 disabled:hover:bg-white"
          @click="pageSelection(albumsPagination.prev._page)"
          :disabled="albumsPagination.prev ? albumsPagination.prev._page === currentPage : true"
        >
          Previous page
        </button>
        <button
          class="border border-1 rounded-lg px-5 py-2 hover:text-white hover:bg-purple-700 disabled:cursor-not-allowed disabled:hover:no-underline disabled:hover:text-gray-300 disabled:text-gray-300 disabled:hover:bg-white"
          @click="pageSelection(albumsPagination.next._page)"
          :disabled="albumsPagination.next ? albumsPagination.next._page === currentPage : true"
        >
          Next page
        </button>
        <button
          ...
          :disabled="albumsPagination.last ? albumsPagination.last._page === currentPage : true"
        >
          Last page
        </button>
      </div>
